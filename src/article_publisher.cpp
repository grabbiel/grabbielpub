...
parse_metadata(const fs::path &metadata_path) {
  ...
  if (!infile.is_open()) {
    log_to_file("Error: Cannot open metadata file at " + metadata_path.string());
    return metadata;
  }
  ...
  for (const auto& key : {"title", "slug", "body_markdown", "site_id"}) {
    if (metadata.find(key) == metadata.end()) {
      log_to_file("Error: Required key missing from metadata: " + std::string(key));
    }
  }
...
bool store_file_reference(int content_id, const std::string &file_type, const std::string &file_path) {
  sqlite3 *db;
  ...
  sqlite3_stmt *stmt;
  const char *sql = 
    "INSERT INTO content_files (content_id, file_type, file_path, is_main) VALUES (?, ?, ?, 0);";
    
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    log_to_file("Error preparing insert statement: " + std::string(sqlite3_errmsg(db)));
    sqlite3_close(db);
    return false;
  }
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    log_to_file("Error inserting file reference: " + std::string(sqlite3_errmsg(db)));
...
// Function to recursively upload files to GCS
bool recursive_upload(const fs::path &dir_path, int content_id) {
  std::string script_path = "/tmp/gcs_upload_" + std::to_string(content_id) + ".sh";
  ...
  std::string gcs_base_path = public_access ? "gs://grabbiel-media-public" : "gs://grabbiel-media";
  std::string gcs_content_path = gcs_base_path + "/articles/" + std::to_string(content_id);
  
  script_file << "#!/bin/bash\n";
  script_file << "# Script to upload article files to GCS\n";
  script_file << "set -e\n\n";
  script_file << "cd " << dir_path.string() << "\n\n";
  script_file << "# Create the base directory structure\n";
  script_file << "sudo gsutil ls " << gcs_content_path << " > /dev/null 2>&1 || sudo gsutil mkdir -p " << gcs_content_path << "\n\n";
  script_file << "echo 'Uploading files...'\n";
  script_file << "find . -type f -name '*.jpg' -o -name '*.png' -o -name '*.gif' | grep -v 'metadata.txt' | while read -r file; do\n";
  script_file << "  dir=$(dirname \"$file\")\n";
  script_file << "  if [ \"$dir\" != \".\" ]; then\n";
  script_file << "    sudo mkdir -p \"/tmp/gcs-upload/" << content_id << "/$dir\"\n";
  script_file << "    cp \"$file\" \"/tmp/gcs-upload/" << content_id << "/$file\"\n";
  script_file << "    sudo gsutil cp -r \"/tmp/gcs-upload/" << content_id << "/$dir\" \"" << gcs_content_path << "/\"\n";
  script_file << "  else\n";
  script_file << "    sudo gsutil cp \"$file\" \"" << gcs_content_path << "/\"\n";
  script_file << "  fi\n";
  script_file << "done\n\n";
  
  if (public_access) {
    script_file << "echo 'Setting public access...'\n";
    script_file << "sudo gsutil -m acl ch -r -u AllUsers:R \"" << gcs_content_path << "/\"\n";
  }
  ...
  std::string exec_cmd = "sudo bash " + script_path + " 2>&1";
  ...
  // Clean up temp files
  std::string cleanup_cmd = "sudo rm -rf /tmp/gcs-upload/" + std::to_string(content_id);
  system(cleanup_cmd.c_str());
  
  // Check if upload was successful
  std::string verify_cmd = "sudo gsutil ls " + gcs_content_path + " 2>/dev/null";
...
bool store_article_files(const fs::path &article_dir, int content_id) {
  log_to_file("Storing article files from " + article_dir.string() + " for content ID " + std::to_string(content_id));
  ...
  try {
    ...
    for (const auto &entry : fs::recursive_directory_iterator(article_dir)) {
      ...
      // Copy the file
      log_to_file("Copying file: " + entry.path().string() + " to " + dest_path.string());
      ...
      // Store file reference in database
      log_to_file("Storing reference in database: " + rel_path.string() + " with type: " + file_type);
    ...
    log_to_file("GCS upload " + std::string(gcs_success ? "successful" : "failed"));
...
bool update_article_metadata(
  ...
  const std::vector<std::string> required_keys = {
      "title", "slug", "body_markdown", "site_id"
  };
  ...
  log_to_file("  body_md length: " + std::to_string(body_md.length()) + " chars");

  sqlite3 *db;
  if (sqlite3_open(DB_PATH.c_str(), &db) != SQLITE_OK) {
    log_to_file("Failed to open database at " + DB_PATH + ": " + std::string(sqlite3_errmsg(db)));
    return false;
  }
  ...
  if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    sqlite3_finalize(stmt);
    log_to_file("Found existing content with ID: " + std::to_string(content_id));
    ...
    if (sqlite3_prepare_v2(db, insert_cb_sql, -1, &stmt, nullptr) != SQLITE_OK) {
      log_to_file("SQL prepare error: " + std::string(sqlite3_errmsg(db)));
      sqlite3_close(db);
      return false;
    }
    ...
    if (sqlite3_prepare_v2(db, insert_article_sql, -1, &stmt, nullptr) != SQLITE_OK) {
      log_to_file("SQL prepare error: " + std::string(sqlite3_errmsg(db)));
      sqlite3_close(db);
      return false;
    }
  ...
  if (sqlite3_exec(db, "COMMIT;", nullptr, nullptr, nullptr) != SQLITE_OK) {
    log_to_file("Failed to commit transaction: " + std::string(sqlite3_errmsg(db)));
    sqlite3_close(db);
    return false;
  }
...
void handle_publish_request(const HttpRequest &req, HttpResponse &res) {
  ...
  // If neither is available, return an error
    ...
    res.send(400, "Missing path parameter. Provide it either as a query parameter '?path=' or in the request body.");
    return;
  }
  ...
  log_to_file("Article published successfully with ID: " + std::to_string(content_id));
